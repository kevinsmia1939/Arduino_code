...
// ---------------- Calibration ----------------
// Your board runs ~2.5% slow: 120 target -> 117 measured => target/measured ≈ 1.02564
const float CLOCK_CORR = 1.002f;   // multiply step frequency by this

// ---------------- Speed control (rotational frequency) ----------------
volatile float Hz = 1.0f;      // rotations per second (1 Hz = 60 RPM)
...
volatile float targetHz = 1.0f;   // what the encoder sets (command)
float currentHz = 0.0f;           // what we actually apply to the timer (ramped)

// Ramp tuning (in Hz/s). 1 Hz = 60 RPM
const float ACCEL_HZ_PER_S = 5.0f;   // how fast it can speed up
const float DECEL_HZ_PER_S = 8.0f;   // how fast it can slow down (often can be higher)

// How often to update the ramp (ms)
const uint16_t RAMP_UPDATE_MS = 5;
unsigned long lastRampUpdate = 0;

// Optional: don't spam Timer1 writes for tiny changes
...
void applyHz(float hz_rps) {
  // Apply to timer only (do NOT treat this as the user target)
...
void setup() {
  ...
  targetHz  = 1.0f;     // or whatever you want as startup
  currentHz = 0.0f;     // start from standstill for a gentle ramp
  lastAppliedHz = -999.0f;

  applyHz(0.0f);        // ensure stopped

}

void loop() {
  // --- Encoder rotation: +/-0.1 Hz per detent ---
  int clkState = digitalRead(ENC_CLK);
  if (clkState != lastCLK) {
    if (digitalRead(ENC_DT) != clkState) Hz += HZ_STEP; else Hz -= HZ_STEP;
    if (Hz < HZ_MIN) Hz = HZ_MIN;
    if (Hz > HZ_MAX) Hz = HZ_MAX;
    Hz = roundf(Hz * 10.0f) / 10.0f;  // keep single decimal
    applyHz(Hz);
    lastCLK = clkState;
  }
  ...
  if (now - lastLcdUpdate >= lcdIntervalMs) {
    ...
    long tenths = lround(Hz * 10.0f);
    if (tenths != lastShownTenths || running != lastShownRun) {
      float rpm = computeRPM(Hz);
